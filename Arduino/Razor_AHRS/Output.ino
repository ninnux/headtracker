...
void my_output_angles(float my_yaw_zero)
{
  if (output_format == OUTPUT__FORMAT_BINARY)
  {
    float ypr[3];  
  ...
  {
   float pippo=0;
   pippo=TO_DEG(yaw)-TO_DEG(my_yaw_zero);
   if(abs(pippo)>180 & TO_DEG(my_yaw_zero) >0 ) pippo=(360-abs(pippo));
   if(abs(pippo)>180 & TO_DEG(my_yaw_zero) <0 ) pippo=-1*(360-abs(pippo));
    Serial.print("#YPRnino=");
    //Serial.print(TO_DEG(my_yaw_zero)); Serial.print(",");
    Serial.print(pippo); Serial.print(",");
    //Serial.print(TO_DEG(yaw)); Serial.print(",");
    Serial.print(TO_DEG(pitch)); Serial.print(",");
    Serial.print(TO_DEG(roll)); Serial.println();
    
    int pulseWidthpippo = map(pippo, -180,180, MIN_CHANNEL_PULSE, MAX_CHANNEL_PULSE);
    int pulseWidthpitch = map(pitch, -180,180, MIN_CHANNEL_PULSE, MAX_CHANNEL_PULSE);
    encoderWrite(0, pulseWidthpippo);
    encoderWrite(1, pulseWidthpitch);
  }
}
